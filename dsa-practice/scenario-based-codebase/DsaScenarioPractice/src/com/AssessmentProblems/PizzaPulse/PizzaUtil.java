package com.AssessmentProblems.PizzaPulse;
...
public class PizzaUtil {
    ...
    public boolean validateOrderId(String orderId)
        ...
        if (!orderId.matches(regex)) {
            throw new InvalidPizzaOrderException(
                "The order ID " + orderId + " is invalid");
        }
        return true;
    }

    public boolean validatePizzaType(String pizzaType)
            throws InvalidPizzaOrderException {
    ...
    public double calculateFinalBill(String pizzaType,
                                     double basePrice,
                                     int quantity)
            throws InvalidPizzaOrderException {
        ...
        if (basePrice < 200) {
            throw new InvalidPizzaOrderException(
                "Invalid base price for " + pizzaType);
        }

        double toppingPercent = 0;

        switch (pizzaType) {
            case "Margherita": toppingPercent = 10; break;
            case "Pepperoni": toppingPercent = 20; break;
            case "Veggie": toppingPercent = 15; break;
            case "BBQ Chicken": toppingPercent = 25; break;
        }
